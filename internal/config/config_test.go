...
func TestLoadFromEnvParsesDurableMaintenanceBounds(t *testing.T) {
	...
	t.Setenv("STELE_JOBS_DURABLE_MAINTENANCE_ENABLED", "true")
	cfg, err := LoadFromEnv()
	if err != nil {
		t.Fatalf("LoadFromEnv() error = %v", err)
	}
	if !cfg.Jobs.DurableMaintenanceEnabled || cfg.Jobs.MaintenanceLeaseDuration != 2*time.Minute || cfg.Jobs.MaintenanceMaxAttempts != 4 {
