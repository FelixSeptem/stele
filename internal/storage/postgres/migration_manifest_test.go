...
func TestMigrationManifestIsDeterministicAndChecksummed(t *testing.T) {
	...
	if len(first) != 17 {
		t.Fatalf("manifest length = %d, want 17: %+v", len(first), first)
	}
	if first[0] != second[0] {
	...
	if first[9].Version != 10 || first[9].Name != "0010_projection_freshness.up.sql" {
		t.Fatalf("manifest entry = %+v, want version 10 projection freshness", first[9])
	}
	if first[10].Version != 11 || first[10].Name != "0011_provider_runtime_bindings.up.sql" {
		t.Fatalf("manifest entry = %+v, want version 11 provider runtime bindings", first[10])
	}
	if first[11].Version != 12 || first[11].Name != "0012_provider_lifecycle_idempotency.up.sql" {
		t.Fatalf("manifest entry = %+v, want version 12 provider lifecycle idempotency", first[11])
	}
	if first[12].Version != 13 || first[12].Name != "0013_query_adaptive_retrieval_planning.up.sql" {
		t.Fatalf("manifest entry = %+v, want version 13 query-adaptive retrieval planning", first[12])
	}
	...
	if first[15].Version != 16 || first[15].Name != "0016_context_calibration_summaries.up.sql" {
		t.Fatalf("manifest entry = %+v, want version 16 context calibration summaries", first[15])
	}
	if first[16].Version != 17 || first[16].Name != "0017_context_calibration_rollout.up.sql" {
		t.Fatalf("manifest entry = %+v, want version 17 context calibration rollout", first[16])
	}
