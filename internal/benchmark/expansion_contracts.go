...
func BuiltinSpecializedCases(scope memory.Scope) []SpecializedCase {
	return []SpecializedCase{
		...
		{ID: "multi-hop-v1", Subfamily: "multi-hop", Events: []MemoryEventRecord{{ID: "hop-a", Scope: scope, Class: memory.MemoryClassRelation, Text: "Ada maintains the Stele service.", ExpectedState: memory.MemoryStateActive}, {ID: "hop-b", Scope: scope, Class: memory.MemoryClassProcedural, Text: "Stele stores memory in PostgreSQL.", ExpectedState: memory.MemoryStateActive}}, Query: BenchmarkQuery{ID: "multi-hop-q1", Scope: scope, Text: "Which database backs Ada's memory service?", QueryType: "multi-hop", EvidenceGroups: []EvidenceGroup{{ID: "hop-g", EvidenceIDs: []string{"hop-a", "hop-b"}, Required: true}}}, QRELs: []QREL{{QueryID: "multi-hop-q1", EvidenceID: "hop-a", Grade: 2, Role: "supporting", GroupID: "hop-g"}, {QueryID: "multi-hop-q1", EvidenceID: "hop-b", Grade: 3, Role: "supporting", GroupID: "hop-g"}}},
	}
}
