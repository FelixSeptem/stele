...
The evaluator SHALL compare the flat fusion baseline with three progressive
context levels: a short retrieval projection, a medium session or context
overview, and canonical or chunk evidence. Each level MUST report source
watermark, freshness category, token or character budget, citation/evidence
coverage, deterministic rebuild identity, and lifecycle/scope safety. Generated
summaries MUST remain derived artifacts and MUST NOT become canonical memory.
...
- **THEN** the report contains separate level identities and bounded metrics for
  recall, evidence coverage, cost, freshness, citations, and rebuildability
...
- **THEN** the level produces deterministic content identity and item order while
  preserving prior derived versions as append-only history
...
hard safety gates, protected quality thresholds, advisory metrics, resource
bounds, prerequisite evidence, rebuild/re-index procedures, rollback steps,
retention ownership, and threshold review cadence. Every report and decision
MUST identify the policy version. Experimental strategies MUST remain disabled
for default retrieval until all required gates pass; policy changes MUST create a
new policy version.

#### Scenario: Candidate passes every required gate

- **WHEN** a candidate has compatible evidence, zero safety failures, preserved
  protected coverage, acceptable latency and budgets, fresh rebuildable context,
...
- **WHEN** an operator changes a threshold, protected category, bound,
  prerequisite, retention rule, or rollback condition
- **THEN** the policy version increments and future reports cannot be compared as
  compatible with the previous policy without an explicit migration decision
