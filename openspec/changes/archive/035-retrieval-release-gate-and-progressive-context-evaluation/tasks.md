...
- [x] 1.1 Add failing tests for release-report identity compatibility across
...
- [x] 1.2 Define versioned release-policy, protected-threshold, resource-budget,
...
- [x] 1.3 Add explicit owned evaluation DSN/provider-profile configuration and
...
- [x] 2.1 Extend replay orchestration to run canonical-v1/baseline-v1 and
...
- [x] 2.2 Add release reports for protected recall, temporal and multi-hop
...
- [x] 2.3 Add tests proving skipped, stale, incompatible, or failing real-stack
...
- [x] 6.1 Add release checklist and rebuild/re-index/rollback runbooks for chunks,
  embeddings, duplicate clusters, projections, and parent-first artifacts.
- [x] 6.2 Add CI smoke coverage using repository-owned fixtures and an opt-in
  release job for the owned real-provider gate; ensure skipped evidence is never
  reported as a pass.
- [x] 6.3 Document local `.env.local` provider/evaluation placeholders, report
  redaction, retention, threshold ownership, and operational failure handling.
- [x] 6.4 Add low-cardinality release-gate, trajectory, integrity, cleanup, and
  rollback telemetry/admin diagnostics consistent with existing observability.

## 7. Verification and OpenSpec completion

- [x] 7.1 Run focused retrieval, projection, benchmark, config, storage, admin,
  and retention tests uncached; include PostgreSQL 18 + pgvector when the owned
  DSN is configured.
- [x] 7.2 Run `go test ./... -count=1 -timeout 15m`, available quality gates,
  `git diff --check`, and the template-marker scan; record unavailable environment
  prerequisites without claiming release passage.
- [x] 7.3 Run race tests where the host toolchain supports them and capture any
  exact environmental limitation. The host has `D:\tools\mingw64\bin\gcc.exe`,
  but Go's Windows `cgo.exe` exits with status 2 during `go test -race`; the
  full race gate remains a CI/toolchain verification requirement.
- [x] 7.4 Run strict OpenSpec validation for this change and `openspec validate
  --all`; update roadmap evidence only to the state actually demonstrated.
  Roadmap records implementation complete while explicitly retaining the
  owned PostgreSQL + pgvector evidence prerequisite for release approval.
